...
## 1. Problem
The calendar's text colors did not adapt to light and dark themes correctly. This was caused by code in `MainActivity.kt` that programmatically created and styled the calendar, overriding the XML themes and styles. This made the component difficult to debug and maintain. The repeated issues led to the decision to fully remake the calendar component.

## 2. Solution: Refactoring to XML
Instead of patching the programmatic code, a full refactoring of the component was performed, replacing the old system with a robust, declarative XML-based implementation.

### Step 1: Remove Programmatic Logic
All functions related to the programmatic creation and styling of the calendar were removed from `MainActivity.kt`. This includes:
-   `setupCalendarView()`
-   `updateDateInfoText()`
...
Calls to `setupCalendarView()` in `onCreate()` and `showHomeScreen()` were also removed to completely disable the old logic. All debugging logs related to the old programmatic calendar were also removed.

### Step 2: Update the XML Layout
In `app/src/main/res/layout/activity_main.xml`, the old `LinearLayout` placeholder (`@+id/calendarPlaceholder`) was removed and replaced with a standard `CalendarView` widget:
```xml
...
A single, clean theme overlay style was created in `app/src/main/res/values/styles.xml` to control the `CalendarView`'s appearance. The old, conflicting calendar styles were all removed.
...
## 3. Conclusion
This comprehensive refactoring removes the source of past theme bugs and aligns the calendar component with standard Android development best practices. The calendar's appearance is now defined declaratively in XML, making it robust, maintainable, and correctly responsive to the application's light and dark themes. This is the definitive fix for the reported issues.